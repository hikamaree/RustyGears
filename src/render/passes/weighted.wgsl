...
fn get_lighting(in: VertexOutput) -> vec3<f32> {
    ...
    return color_accum * object_color.rgb;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let color = get_lighting(in);
    let alpha = alpha_sample.r;

    let view_depth = in.view_depth;
    
    let weight = alpha * max(0.001, 1.0 - view_depth * 0.1);
