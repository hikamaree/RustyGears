...
fn vs_main(model: VertexInput, instance: InstanceInput) -> VertexOutput {
    ...
    out.shadow_position = out.clip_position;
    return out;
}

fn get_shadow(shadow_pos: vec4<f32>) -> f32 {
    let proj_coords = shadow_pos.xyz / shadow_pos.w;
    let flip_correction = vec2<f32>(0.5, -0.5);
    let shadow_uv = proj_coords.xy * flip_correction + vec2<f32>(0.5, 0.5);
    let shadow_z = proj_coords.z;
    return textureSampleCompare(t_shadow, s_shadow, shadow_uv, shadow_z - 0.001);
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let shadow = get_shadow(in.shadow_position);
